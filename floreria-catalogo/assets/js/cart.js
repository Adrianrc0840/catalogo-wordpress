...
(function () {
    ...
    /* ── FAB de WhatsApp: contacto directo, sin llenar el formulario ── */
    function createWhatsappFab() {
        var num = getWhatsapp();
        if (!num) return; // sin número configurado no tiene sentido mostrarlo

        var msg = (typeof cartData !== 'undefined' && cartData.waFabMensaje) || '';
        var a   = document.createElement('a');
        a.id        = 'fc-wa-fab';
        a.className = 'fc-wa-fab';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Un solo corte para todos los botones flotantes. El servidor ya resolvió
        // si esta pantalla los admite (páginas desactivadas en Configuración +
        // pantallas internas: PDV, panel, asistencia y rastreo).
        // El chequeo del panel se conserva como respaldo por si llegara a
        // renderizarse por una vía donde el query var no esté disponible.
        if (document.querySelector('.fc-panel-body')) return;
        if (cartData.hideCart) return;
        createFab();
        updateFab();
        createWhatsappFab();
    });
